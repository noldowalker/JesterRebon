...
#endif
[ExecuteAlways]
public class SpringArm : BaseSystem
{
    [Space]
    ...
    public float minClampRotation;
    public float maxClampRotation;
    ...
    public void Rotate(float newYaw, float newPitch)
    {
        ...
        yaw += newYaw * mouseSensitivity * Time.deltaTime;
        // Decrement pitch by Mouse Y input
        pitch += newPitch * mouseSensitivity * Time.deltaTime;
        // Clamp pitch so that we can't invert the the gameobject by mistake
        pitch = Mathf.Clamp(pitch, minClampRotation, maxClampRotation);
